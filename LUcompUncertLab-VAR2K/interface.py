...
class interface(object):
    ...
    def buildDataForModel(self):
        import numpy as np
        y = np.array(self.fluhosp[self.includedstates]).T
        self.y = y
        return y
    ...
    def writeData(self,writeout,dataPredictions,dataQuantiles):
        if dataPredictions is not None:
            ...
            quantileString = "{:s}-LUcompUncertLab-VAR2K-preprocess.csv".format(self.forecast_date)
    ...
    def accessPredictionsAndQuantiles(self):
        ...
        mostRecentFile = sorted(glob("*VAR2*.csv"))[-1]
